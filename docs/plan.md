...
| 3 | **CHIRP 4.3.1** | Version, changelog (Unreleased → 4.3.1), tag, GitHub release from main. `npm publish` needs the user's OTP. Done when npm shows 4.3.1. | ◐ tagged + GitHub release 2026-09-05; `npm publish` awaits the user's OTP |
| 4 | **Write-up** | `docs/paper/` — the telestrator pattern as a paper (abstract, the pattern, the worked example, what was learned, references) ready for Zenodo; `CITATION.cff`; a `v0.1.0` release tag so Zenodo's GitHub integration can mint a DOI; a LinkedIn launch draft in `docs/launch/` in the product-launch voice. Done when the user has three files to read and one button to press. | ☑ 2026-09-05 — paper, CITATION.cff + .zenodo.json, v0.1.0 release, LinkedIn draft |
